...
const TokenMetrix = ({ dataA, allocations, decimals, totalSupply }) => {
  ...
  const allocationAmounts =
    ...
    allocations.map((v, e) => {
      console.log(
        ((formatUnits(v.Amount, decimals) / totalSupply) * 100).toLocaleString(
          "fullwide",
          { useGrouping: false }
        )
      );
      return {
        label: v.Title,
        value: Number((formatUnits(v.Amount, decimals) / totalSupply) * 100),
      };
    });
  ...
  const amountsOnly =
    allocations &&
    allocations.map((v, e) =>
      Number((formatEther(v.Amount) * 10 ** decimals) / 1000000000000000000)
    );
  ...
  allocations &&
    allocationAmounts.push({
      label: "Liquidity",
      value: Number((formatEther(dataA[4][0]) / totalSupply) * 100),
    });
  allocations &&
    allocationAmounts.push({
      label: "UnLocked",
      value: Number(((totalSupply - locked) / totalSupply) * 100),
    });

  const series =
    allocations &&
    allocations.map((v, e) =>
      Number((formatUnits(v.Amount, decimals) / totalSupply) * 100)
    );
  ...
  return (
    <div className="bg-white dark:bg-dark-400 border dark:border-lightDark rounded-md shadow-xl">
      ...
      <div className="py-4 flex justify-center  px-4">
        <DonutChart
          className={`${
            theme === "dark" ? "root-innertext-label" : "root-innertext-label2"
          }`}
