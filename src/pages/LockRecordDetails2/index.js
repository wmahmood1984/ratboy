...
import { rpcObj, tokenLockLauncherAbi, tokenLocklauncherAdd } from "../../config";
const LockRecordDetails2 = () => {
  ...
  const web3 = new Web3(new Web3.providers.HttpProvider(`${rpcObj[`${splittedParams[1]}`]}`))
  const contract = new web3.eth.Contract(tokenLockLauncherAbi,tokenLocklauncherAdd[`${splittedParams[1]}`])
  ...
  useEffect(()=>{
    const abc = async ()=>{
      ...
      const filteredArray = tx1.filter(item=>item._contract==splittedParams[0])
        ...
        <div className="p-4">
          <div className="grid gap-y-4 mt-4">
            <ListItem
              ...
              refA={
                chainId == "97" ? 
                `https://testnet.bscscan.com/token/${state && state.token}` :
                `https://goerli.etherscan.io/token/${state &&  state.token}`
              ...
              color={"primary"}
            />{" "}
            <ListItem title={"Token Name"} desc={`${state && state.name}`} />
            <ListItem title={"Token Symbol"} desc={`${state && state.symbol}`} />
